...
def generic_doc_for_models() -> None:
    ...
    1. to obtain uncalibrated (raw) geometric waveform
            t, h = generate_surrogate(q=8, modes=[(2,1),(2,2),(3,1),(3,2),(3,3),(4,2),(4,3),(4,4),
                  (5,3),(5,4),(5,5)], calibrated=False)
    ...
    4. to obtain NR calibrated physical waveform on a sphere
            t, h = generate_surrogate(q=q, modes=mode_list, M_tot=50, dist_mpc=100, orb_phase=np.pi/3, 
                                      inclination=np.pi/4)
    5. to obtain NR calibrated physical waveform on a sphere for modes up to l=5
            t, h = generate_surrogate(q=q, modes=mode_list, M_tot=50, dist_mpc=100, orb_phase=np.pi/3, 
                                      inclination=np.pi/4, lmax=5)
    6. to obtain mode-summed NR calibrated physical waveform on a sphere
            t, h = generate_surrogate(q=8, M_tot=60, dist_mpc=100, orb_phase=np.pi/3, inclination=np.pi/4, 
...
def BHPTNRSur1dq1e4_doc() -> None:
    ...
    This surrogate model is trained on waveform data generated by point-particle black hole
    perturbation theory (ppBHPT), and tuned to NR simulations in the comparable mass ratio 
    regime (q=3 to q=10). 
